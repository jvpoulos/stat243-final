...
testMain(dbetatest, D = c(0.1,0.9), a=NA, b=NA, n=10000, x)
...
#define gamma(2,2) pdf
pgammatest <- function(x){
  y <- pgamma(x,shape=2, rate=2)
  return(y)
}
...
#define weibull(1.5,1) pdf
pweibulltest <- function(x){
...
#define logis(5,2) pdf
plogistest <- function(x){
  y <- plogis(x,location = 5, scale=2)
  return(y)
}
